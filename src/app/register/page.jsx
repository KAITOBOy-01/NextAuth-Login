...
function RegisterPage() {
    ...
    // When got seesion redirect to welcome
    const { data: session } = useSession("");
    if (session) redirect("/welcome");

    // check error password and email
    const handleSubmit = async (e) => {
        e.preventDefault();

        if (password != confirmPassword) {
            setError("Passsword is not match!")
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                const form = e.target;
                setError("");
                setSuccess("User registration suscessfully!")
                form.reset();
            }else {
                console.log("User rregistration failed.")
            }
        } catch(error){
    ...
    return (
        <div >
            ...
            <div className='container mx-auto py-4'>
                ...
                <hr className='my-3 h-0.5 opacity-20' />
                <p>Already have an accout ? go to <Link className='text-blue-500 hover:underline' href="/login">Login</Link> Page</p>
            </div>
        </div>
    )
}
